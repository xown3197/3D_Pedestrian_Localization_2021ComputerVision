...
from monoloco.utils.potenit import get_calibration_potenit
import os
import glob
import copy
import logging
...
from ..utils import get_calibration, split_training, parse_ground_truth, get_iou_matches, append_cluster, get_calibration_potenit, split_training_potenit
from ..network.process import preprocess_pifpaf, preprocess_monoloco


class PreprocessPotenit:
    ...
    def __init__(self, dir_ann, iou_min):
        ...
        self.dir_gt = os.path.join('data', 'potenit', 'gt')
        self.names_gt = tuple(os.listdir(self.dir_gt))
        self.dir_kk = os.path.join('data', 'potenit', 'calib')
        ...
        self.path_joints = os.path.join(dir_out, 'joints-potenit-' + now_time + '.json')
        self.path_names = os.path.join(dir_out, 'names-potenit-' + now_time + '.json')
        path_train = os.path.join('splits', 'potenit_train.txt')
        path_val = os.path.join('splits', 'potenit_test.txt')
        self.set_train, self.set_val = split_training_potenit(self.names_gt, path_train, path_val)
    ...
    def run(self):
        ...
        for name in sorted(self.names_gt):
            ...
            p_left, _ = get_calibration_potenit()
            kk = p_left[0]
            ...
            try:
                with open(os.path.join(self.dir_ann, 'I0'+ basename + '.png.predictions.json'), 'r') as f:
                    annotations = json.load(f)
                boxes, keypoints = preprocess_pifpaf(annotations, im_size=(570, 455))
                if keypoints==[]: raise FileNotFoundError
