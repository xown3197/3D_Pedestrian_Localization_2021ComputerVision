...
from ..utils import get_iou_matches, get_task_error, get_pixel_error, check_conditions, get_category, split_training, \
    parse_ground_truth, split_training_potenit
...
class EvalPotenit:
    ...
    CLUSTERS = ('easy', 'moderate', 'hard', 'all')
    # ALP_THRESHOLDS = ('<0.5m', '<1m', '<2m')
    ALP_THRESHOLDS = ('<0.05m', '<0.1m', '<0.25m', '<0.5m', '<1m', '<2m')
    ...
    # BASELINES = [ ]
    HEADERS = ('method', '<0.05', '<0.1', '<0.25', '<0.5', '<1m', '<2m', 'easy', 'moderate', 'hard', 'all')
    ...
    def __init__(self, thresh_iou_monoloco=0.3, thresh_iou_base=0.3, thresh_conf_monoloco=0.3, thresh_conf_base=0.3,
        ...
        _, self.set_val = split_training_potenit(names_gt, path_train, path_val)
    ...
    def update_errors(self, dd, dd_gt, cat, errors):
        ...
        if diff <= 0.25:
            errors['<0.25m'].append(1)
        else:
            errors['<0.25m'].append(0)

        if diff <= 0.1:
            errors['<0.1m'].append(1)
        else:
            errors['<0.1m'].append(0)

        if diff <= 0.05:
            errors['<0.05m'].append(1)
        else:
            errors['<0.05m'].append(0)
    ...
    def summary_table(self, all_methods):
        ...
        aa = [[f'{key}:{len(self.errors[key][perc])}' for perc in list(self.ALP_THRESHOLDS)] for key in all_methods]
        
        # for perc in ['<0.5m', '<1m', '<2m']]
        alp = [[str(100 * average(self.errors[key][perc]))[:5]
                for perc in list(self.ALP_THRESHOLDS)] 
...
def add_true_negatives(err, cnt_gt):
    ...
    err['<1m'].extend(zeros)
    err['<2m'].extend(zeros)
    err['<0.25m'].extend(zeros)
    err['<0.1m'].extend(zeros)
    err['<0.05m'].extend(zeros)
...
if __name__=="__main__":
    potenit_eval = EvalPotenit()
    potenit_eval.run()
    potenit_eval.printer(show=True, save=False)
