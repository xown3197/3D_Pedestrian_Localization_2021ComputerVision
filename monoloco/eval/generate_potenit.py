...
class GeneratePotenit:
    ...
    def __init__(self, model, dir_ann, p_dropout=0.2, n_dropout=0, stereo=True):
        ...
        dir_gt = os.path.join('data', 'potenit', 'gt')
        ...
        if stereo:
            ...
            self.dir_images = os.path.join('data', 'potenit', 'images')
            self.dir_images_r = os.path.join('data', 'potenit', 'images_r')

    def run(self):
        """Run Monoloco and save txt files for KITTI evaluation"""

        cnt_ann = cnt_file = cnt_no_file = 0
        dir_out = {"monoloco": os.path.join('data', 'potenit', 'monoloco')}
        make_new_directory(dir_out["monoloco"])
        print("\nCreated empty output directory for txt files")

        if self.stereo:
            for key in self.baselines:
                dir_out[key] = os.path.join('data', 'potenit', key)
        ...
        print("\nSaved in {}/{} txt {} annotations. Not found {} images fuck {}".format(cnt_file, len(self.set_basename), cnt_ann, cnt_no_file, pass_cnt))
    ...
    def _run_stereo_baselines(self, basename, boxes, keypoints, zzs, path_calib):
        ...
        annotations_r, _, _ = factory_file(self.dir_ann, f'I0{basename}', mode='right')
...
def factory_basename(dir_ann, dir_gt):
    ...
    _, set_val_gt = split_training_potenit(names_gt, path_train, path_val)
    ...
    set_basename = {f'{int(os.path.basename(x).split(".")[0][1:]):06d}' for x in list_ann}
    # import pdb;pdb.set_trace()
